...
Set `ChatRequest.captureRequestPayload` to `true` alongside `trace` and `onTraceEvent` to receive `provider_attempt_payload` for every adapter-level attempt. It contains the adapter's HTTP JSON body or SDK input object as a defensive snapshot, including prompts and tool definitions; it does not contain authentication headers. This is private data: callers must explicitly opt in and protect any persisted copy. SDKs may further transform their input or retry internally before sending it, so this is not a byte-for-byte wire capture or a guarantee of one event per network request. If payload capture fails, an `unavailable` event is emitted without interrupting the request. Standard tracing leaves this event disabled.
